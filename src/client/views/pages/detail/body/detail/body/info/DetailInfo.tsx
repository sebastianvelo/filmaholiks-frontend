...
const DetailInfo: FunctionComponent<DetailInfoProps> = (props: DetailInfoProps) => (
    <div className={`md:col-span-2 grid grid-cols-2 sm:grid-cols-4 w-full gap-y-4 place-content-between px-2 py-4`}>
        {props.data?.filter(info => info?.title).map((info) => (
            <Container className="border-l-4 border-secondary dark:border-primary flex flex-col justify-center" key={info?.title}>
