...
const NavigationActions: React.FC<NavigationActionsProps> = (props: NavigationActionsProps) => {
  ...
  const wrapperClassName = Tailwind.builder()
    .add("bg-primary-100/80 backdrop-blur-2xl xl:backdrop-blur-none")
    .add("dark:bg-primary-950/70 xl:dark:bg-transparent")
    .add("flex-col absolute h-screen space-y-4 w-2/3 md:w-1/2 xl:w-full top-12 left-0")
  ...
  return (
    <div className={className}>
      <div className={wrapperClassName}>
        <div className="px-4 xl:px-6 flex flex-col xl:flex-row">
          {props.actions?.map((action: ActionProps) => (
            <Action key={action.label} {...action} className={actionClassName} />
          ))}
        </div>
        <div className="absolute bottom-4 left-4 items-center flex xl:relative xl:bottom-0 xl:-left-4 space-x-2">
